...
const products = [
  ...
  { id: 6, name: "Conjunto Infantil", category: "Niñas", subcategory: "Edición Especial", price: 85990, description: "Conjunto elegante y divertido para niñas.", sizes: ["2", "4", "6", "8"], imgs: ["assets/imagenes/ninas/Frente Conjunto Infantil.png"], stock: 12, rating: 4.6, reviews: 31, features: ["Estampado exclusivo", "Material: Algodón y elastano", "Comodidad máxima"] },
  ...
  { id: 8, name: "Set Falda", category: "Niñas", subcategory: "Edición Especial", price: 89990, description: "Set con falda para niñas. Incluye falda y top a juego.", sizes: ["2", "4", "6", "8"], imgs: ["assets/imagenes/ninas/Frente Set Falda.png"], stock: 7, rating: 4.9, reviews: 47, features: ["Falda con vuelo", "Top a juego", "Material ligero"] }
];
...
function loadCategories() {
  const stored = localStorage.getItem('angelow_client_categories');
  if (stored) { categories = JSON.parse(stored); if (!categories.includes("Todos")) categories = ["Todos", ...categories]; }
  renderCategories();
}
...
function logout() {
  if (currentUser && currentUser.id) localStorage.setItem(`angelow_favorites_${currentUser.id}`, JSON.stringify(favorites));
  currentUser = null;
...
function requireAuth(action, callback) {
  if (!currentUser) {
    showToast({ title: "Inicia sesión", message: "Debes iniciar sesión para " + action, type: "warning" });
...
function getFilteredProducts() {
  return products.filter(p => {
    if (activeCategory !== "Todos") {
      if (["Popular", "Edición especial", "Oferta"].includes(activeCategory)) return p.subcategory === activeCategory;
      else return p.category === activeCategory;
    }
    return true;
  }).filter(p => !searchQuery || p.name.toLowerCase().includes(searchQuery) || p.category.toLowerCase().includes(searchQuery) || p.subcategory.toLowerCase().includes(searchQuery));
}

function renderCategories() {
  let html = [...categories, ...categories, ...categories].map(c => `<span class="cat-item ${activeCategory === c ? 'active' : ''}" onclick="setCategory('${c}')">${c}</span>`).join("");
...
function renderProducts() {
  ...
  grid.innerHTML = filtered.map(p => {
    ...
    return `<div class="card-container" onclick="openProductDetail(${p.id})"><div class="card"><div class="my-logo"><img src="${APP_URL}/assets/imagenes/general/logos.png"></div><img src="${p.imgs[0]}" class="product-img"><h2>${p.name}</h2><p>${p.subcategory}</p><div class="stock-info ${stockClass}">${stockText}</div></div><div class="modal"><div class="my-logo"><img src="${APP_URL}/assets/imagenes/general/logos.png"></div><h3>${p.name}</h3><p class="subtitle">conjunto ${p.category}</p><div class="sizes">${p.sizes.map(s => `<div class="size-box ${selectedSizes[p.id] === s ? 'selected' : ''}" onclick="event.stopPropagation(); selectSize(${p.id},'${s}')">${s}</div>`).join("")}</div><div class="price">COP $${p.price.toLocaleString()}</div><div class="stock-info ${stockClass}" style="margin:8px 0;">${stockText}</div><div class="buttons"><button class="buy-btn ${p.stock === 0 ? 'disabled' : ''}" ${p.stock === 0 ? 'disabled' : ''} onclick="event.stopPropagation(); addToCart(${p.id})">Comprar</button><button class="fav-btn ${favorites.includes(p.id) ? 'active' : ''}" onclick="event.stopPropagation(); toggleFavorite(${p.id})"><img src="${APP_URL}/assets/imagenes/general/favoritos.png"></button></div></div></div>`;
  }).join("");
}
...
function renderCart() {
  ...
  if (cart.length === 0) { container.innerHTML = `<div style="text-align:center;padding:100px 20px;">Tu carrito está vacío</div>`; return; }
  container.innerHTML = cart.map(item => `<div class="cart-item"><img src="${item.imgs[0]}"><div class="cart-info"><h4>${item.name}</h4><span>Talla: ${item.selectedSize} • ${item.subcategory}</span><div class="qty"><button onclick="updateQty('${item.cartId}', -1)" ${item.quantity <= 1 ? 'disabled' : ''}>−</button><strong>${item.quantity}</strong><button onclick="updateQty('${item.cartId}', 1)">+</button></div><strong>COP $${(item.price * item.quantity).toLocaleString()}</strong></div><button class="remove" onclick="removeFromCart('${item.cartId}')">×</button></div>`).join("");
}

function addToCart(pid) {
  event.stopPropagation();
  let p = products.find(p => p.id === pid);
  if (p.stock <= 0) { showToast({ title: "Sin stock", message: "Producto no disponible", type: "error" }); return; }
  let size = selectedSizes[pid];
  if (!size) { showToast({ title: "Selecciona talla", message: "Elige una talla", type: "warning" }); return; }
  let cartId = `${pid}-${size}`;
  let existing = cart.find(i => i.cartId === cartId);
  if (existing && existing.quantity + 1 > p.stock) { showToast({ title: "Stock insuficiente", message: "No hay suficientes unidades", type: "warning" }); return; }
...
function updateQty(cartId, delta) {
  ...
  if (newQty > p.stock) { showToast({ title: "Stock insuficiente", type: "warning" }); return; }
  p.stock -= delta;
...
function proceedToCheckout() {
  ...
  } else window.location.href = APP_URL + '/compra';
}
...
function renderFavorites() {
  ...
  list.innerHTML = favProducts.map(p => `<div class="fav-item"><img src="${p.imgs[0]}" class="fav-item-img"><div class="fav-item-info"><h4>${p.name}</h4><span>${p.category} • ${p.subcategory}</span><div style="font-weight:700;font-size:18px;color:var(--primary);margin-top:6px;">COP $${p.price.toLocaleString()}</div><div class="fav-item-stock ${p.stock > 10 ? 'stock-ok' : p.stock > 0 ? 'stock-low' : 'stock-out'}">${p.stock > 10 ? 'En stock' : p.stock > 0 ? 'Pocas unidades' : 'Agotado'}</div></div><div class="fav-actions"><button class="fav-remove-btn" onclick="toggleFavorite(${p.id})">×</button><button class="add-to-cart-btn" onclick="addFavoriteToCart(${p.id})" ${p.stock === 0 ? 'disabled' : ''}>Carrito</button></div></div>`).join('');
}

function addFavoriteToCart(id) {
  let p = products.find(p => p.id === id);
  if (p.stock <= 0) { showToast({ title: "Agotado", message: `${p.name} sin stock`, type: "error" }); return; }
  let size = p.sizes[0];
  let cartId = `${id}-${size}`;
  let existing = cart.find(i => i.cartId === cartId);
  if (existing) existing.quantity++;
...
function openFavorites() {
  if (!currentUser) { showToast({ title: "Inicia sesión", message: "Debes iniciar sesión para ver favoritos", type: "warning" }); window.location.href = APP_URL + `/auth/login?redirect=${encodeURIComponent(window.location.pathname)}`; return; }
...
function openProductDetail(id) {
  ...
  document.getElementById('productDetailContent').innerHTML = `<div class="detail-container"><div class="detail-images"><img src="${p.imgs[0]}" class="detail-main-image" id="detailMainImage"><div class="detail-image-thumbnails">${p.imgs.map((img, i) => `<img src="${img}" class="detail-thumbnail ${i === 0 ? 'active' : ''}" onclick="document.getElementById('detailMainImage').src='${img}'; document.querySelectorAll('.detail-thumbnail').forEach(t => t.classList.remove('active')); this.classList.add('active')">`).join('')}</div></div><div class="detail-info"><div class="detail-category">${p.category}</div><h1 class="detail-name">${p.name}</h1><p class="detail-description">${p.description}</p><div class="detail-rating"><div class="stars">${stars}</div><span class="rating-text">${p.reviews || 0} reseñas</span><a href="#" class="rating-link" onclick="event.preventDefault(); showToast({title:'Reseñas',message:'Función próximamente',type:'info'})">Dejar tu reseña</a></div><div class="detail-prices"><span class="detail-price">COP $${p.price.toLocaleString()}</span></div><div class="detail-size-section"><div class="detail-size-title">Talla</div><div class="detail-sizes">${p.sizes.map(s => `<div class="detail-size-box ${selectedSizes[p.id] === s ? 'selected' : ''}" onclick="selectedSizes[${p.id}]='${s}'; this.parentElement.querySelectorAll('.detail-size-box').forEach(b => b.classList.remove('selected')); this.classList.add('selected')">${s}</div>`).join('')}</div><div class="size-help"><a href="#" class="size-help-link" onclick="event.preventDefault(); showToast({title:'Ayuda',message:'Contáctanos para asesoramiento',type:'info'})">¿No encuentras tu talla? Te ayudamos</a></div></div><div class="detail-ref">REF: ${p.id}</div><div class="detail-actions"><button class="detail-action-btn detail-buy-now" onclick="buyNowProduct(${p.id})">COMPRAR AHORA</button><button class="detail-action-btn detail-add-cart" onclick="addToCartFromDetail(${p.id})">AGREGAR AL CARRITO</button></div><div class="detail-shipping-info"><div class="shipping-title">Envío y Devoluciones</div><div class="shipping-item"><span style="color:#00a650">✓</span>Envío gratis en pedidos > $58.900</div><div class="shipping-item"><span style="color:#00a650">✓</span>Devoluciones gratuitas</div><div class="shipping-item"><span style="color:#00a650">✓</span>Entrega estimada: 5-10 días</div></div><div class="detail-specs"><div class="specs-title">Descripción</div><div class="specs-list">${(p.features || ['Material de alta calidad', 'Diseño exclusivo', 'Comodidad garantizada']).map(f => `<div class="spec-item">${f}</div>`).join('')}</div></div></div></div>`;
...
function buyNowProduct(id) { addToCartFromDetail(id); closeProductDetail(); if (!currentUser) { showToast({ title: "Inicia sesión", message: "Debes iniciar sesión para finalizar tu compra", type: "warning" }); setTimeout(() => window.location.href = APP_URL + `/auth/login?redirect=${encodeURIComponent('/compra')}`, 500); } else setTimeout(() => window.location.href = APP_URL + '/compra', 300); }
function addToCartFromDetail(id) {
  let p = products.find(p => p.id === id);
  let size = selectedSizes[id];
  if (!size && p.sizes?.length) { showToast({ title: "Selecciona talla", message: "Elige una talla", type: "warning" }); return; }
  if (p.stock <= 0) { showToast({ title: "Agotado", message: "Producto no disponible", type: "error" }); return; }
...
// ======================== RASCA PRODUCTOS NUEVOS (CANVAS) ========================
let scratchRevealed = false;
let canvas, ctx, isDrawing = false;
let scratchPixels = 0;
const requiredPixels = 0.6; // 60% del área rascada

function getNewProducts() {
  // Productos con subcategoría "Edición Especial"
...
function initScratchCard() {
  ...
  // Dibujar fondo gris con textura
...
// ======================== INICIALIZACIÓN ========================
document.getElementById('searchInput').addEventListener('input', e => { searchQuery = e.target.value.toLowerCase().trim(); document.getElementById('clearSearch').style.display = searchQuery ? 'block' : 'none'; renderProducts(); });
document.getElementById('clearSearch').onclick = () => { document.getElementById('searchInput').value = ''; searchQuery = ''; document.getElementById('clearSearch').style.display = 'none'; document.getElementById('searchInput').focus(); renderProducts(); };
document.getElementById('cartBtnHeader').onclick = openCart;
document.getElementById('favBtnHeader').onclick = openFavorites;
document.getElementById('openFavoritesFromMenu').onclick = (e) => { e.preventDefault(); document.getElementById('dropdownMenu').classList.remove('show'); openFavorites(); };
document.getElementById('closeFavorites').onclick = closeFavorites;
document.querySelectorAll('.overlay').forEach(o => { o.onclick = e => { if (e.target === o) o.classList.remove('active'); }; });
document.getElementById('profileBtn').addEventListener('click', (e) => { e.stopPropagation(); document.getElementById('dropdownMenu').classList.toggle('show'); });
document.addEventListener('click', () => { document.getElementById('dropdownMenu').classList.remove('show'); });
document.addEventListener('keydown', (e) => { if (e.key === 'Escape' && document.getElementById('productDetail').classList.contains('active')) closeProductDetail(); });
document.getElementById('currentYear').textContent = new Date().getFullYear();

function checkPendingFavorite() {
  const pending = localStorage.getItem("angelow_pending_favorite");
  if (pending && currentUser) {
    const id = parseInt(pending);
    if (!favorites.includes(id)) {
      favorites.push(id);
      saveFavorites();
      if (currentUser.id) localStorage.setItem(`angelow_favorites_${currentUser.id}`, JSON.stringify(favorites));
      updateFavBadges();
      renderProducts();
...
initScratchCard(); // Inicializar el canvas de rascar
